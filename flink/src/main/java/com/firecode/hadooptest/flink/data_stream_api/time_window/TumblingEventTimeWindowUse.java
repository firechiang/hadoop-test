...
/**
 * 
 * 滚动窗口（按时间切分计算数据）：将指定时间范围内的数据放到指定大小的窗口当中（窗口内不会有相同数据）进行计算
 * 比如：每隔5秒计算一次
 * 
 * Flink 事件时间描述
 ...
 * @author JIANG
 */
public class TumblingEventTimeWindowUse {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 */
		// 滚动窗口（按时间切分数据）：指定时间范围内将数据放到指定大小的窗口当中（窗口内不会有相同数据）进行计算
		WindowedStream<User, Tuple, TimeWindow> window = keyBy.window(TumblingEventTimeWindows.of(Time.seconds(5)));
		// 上面的简写
		//WindowedStream<User, Tuple, TimeWindow> window = keyBy.timeWindow(Time.seconds(5));
