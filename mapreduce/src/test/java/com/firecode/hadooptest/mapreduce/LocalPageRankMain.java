...
import com.firecode.hadooptest.mapreduce.pagerank.PageRankCount;
import com.firecode.hadooptest.mapreduce.pagerank.PageRankMapper;
import com.firecode.hadooptest.mapreduce.pagerank.PageRankReduce;
 ...
 *
 */
public class LocalPageRankMain {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		while(true) {
			...
			job.setJarByClass(LocalPageRankMain.class);
			job.setJobName("pr"+i);
			...
			Path inputPath = new Path("D:/hadoop-test/data/pagerank.txt");
			//第二次开始我们要使用上一次的计算结果作为输入
			if(i > 1 ) {
				inputPath = new Path("D:/hadoop-test/result/pagerank/pr"+(i-1));
			}
			FileInputFormat.addInputPath(job, inputPath);
			
			Path outputPath = new Path("D:/hadoop-test/result/pagerank/pr"+i);
