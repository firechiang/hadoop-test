...
#### 二、下载 scylla 源配置文件到 /etc/yum.repos.d 目录
```bash
...
#### 五、scylla --help 查看配置帮助，修改[vi /etc/sysconfig/scylla-server]启动脚本，添加到 SCYLLA_ARGS 选项里面
```bash
--memory 1G                                            # 每个CPU使用的最大内存(不配置的话，默认是4G*CPU，如果机器内存不足，将导致无法启动Scylla)
