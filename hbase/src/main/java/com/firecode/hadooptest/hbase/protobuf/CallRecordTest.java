...
import com.firecode.hadooptest.hbase.protobuf.CallRecordBuilder.CallRecord;
import com.firecode.hadooptest.hbase.protobuf.CallRecordBuilder.CallRecord.Builder;
...
public class CallRecordTest extends HBaseConnectionTest {
	/**
	 * 创建 call_record1 通话记录表
	 * @throws IOException
	 */
	@Before
	public void create() throws IOException {
		
		TableName tableName = TableName.valueOf("call_record1");
		//表是否存在
	...
	public void insert() throws IOException {
		...
		for(int i=0;i<10;i++) {
			...
			for(int j=0;j<100;j++) {
				...
				Builder builder = CallRecordBuilder.CallRecord.newBuilder();
				builder.setDnumPhone(dnumPhone);
				builder.setLength(length);
				builder.setType(type);
				builder.setDate(dateStr);
				CallRecord callRecord = builder.build();
				put.addColumn(family, Bytes.toBytes("call_record1"), callRecord.toByteArray());
				puts.add(put);
			}
		}
		TableName tableName = TableName.valueOf("call_record1");
